...
float to_rad(float degree) { return degree * PI / 180.0f; }

// rotates world orientation to camera matrix
const Eigen::Matrix3f to_zfront =
...
void Camera::onMouseDrag(int pos_x, int pos_y) {
  onMouseDrag(Eigen::Vector2i(pos_x, pos_y));
}
...
Eigen::Vector2f Camera::absToRelative(const Eigen::Vector2i& delta) const {
  return Eigen::Vector2f(
      static_cast<float>(delta.x()) / static_cast<float>(window_size.x()),
...
void Camera::updateViewMatrix() {
  ...
  view_matrix.topRightCorner<3, 1>() =
      -view_matrix.topLeftCorner<3, 3>() *
...
void Camera::updateProjectionMatrixOrtho() {
  ...
  projection_matrix << (2.0f / horizontal_box_size), 0, 0, 0, 0,
      (1.0f / (horizontal_box_size * inverse_aspect)), 0, 0, 0, 0,
      -2.0f / (far - near), -(far + near) / (far - near), 0, 0, 0, 1;
}
...
void Camera::updateProjectionMatrixPerspective() {
  ...
  projection_matrix << (1.0f / half_width_over_depth), 0, 0, 0, 0,
      (1.0f / (inverse_aspect * half_width_over_depth)), 0, 0, 0, 0,
