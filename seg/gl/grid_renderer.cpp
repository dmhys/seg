...
GridRenderer::GridRenderer() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertex_count[0] * 3,
               lod0_vertices.data(), GL_STATIC_DRAW);

  // lod1 - 20m
  glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
  vertex_count[1] = lod1_vertices.size();
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertex_count[1] * 3,
               lod1_vertices.data(), GL_STATIC_DRAW);

  // lod2 - 5m
  glBindBuffer(GL_ARRAY_BUFFER, vbo[2]);
  vertex_count[2] = lod2_vertices.size();
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertex_count[2] * 3,
               lod2_vertices.data(), GL_STATIC_DRAW);

  // lod3 - 1m
  glBindBuffer(GL_ARRAY_BUFFER, vbo[3]);
  vertex_count[3] = lod3_vertices.size();
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertex_count[3] * 3,
               lod3_vertices.data(), GL_STATIC_DRAW);
}
