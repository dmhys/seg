...
void Controller::init(gl::Scene *_scene,
  ...
  base_objects.push_back(
      std::move(std::unique_ptr<ObjectListWindow>(obj_list)));
  base_objects.push_back(
      std::move(std::unique_ptr<ObjectInspectorWindow>(obj_inspector)));
}
...
void Controller::handleMouseEvents() {
  ...
  for (int i = 0; i < 3; i++)  // 0 left // 1 right // 2 wheel
  {
    if (ImGui::IsMouseClicked(i))
      scene->camera.onMouseClick(i, mouse_pos.x, mouse_pos.y);
    if (ImGui::IsMouseDragging(i))
      scene->camera.onMouseDrag(mouse_pos.x, mouse_pos.y);
  }
