...
void Path::drawImpl() {
  std::lock_guard<std::mutex> lock(pose_mtx);
  if (hasType(type, VisualType::LINE)) line_renderer->draw();
...
void Path::drawInspector() {
  ...
  if (ImGui::BeginCombo("Visualize", enumToCharP(type))) {
    if (ImGui::Selectable(enumToCharP(VisualType::LINE),
    ...
    if (ImGui::Selectable(enumToCharP(VisualType::CAMERA_FRAMES),
                          type == VisualType::CAMERA_FRAMES))
      type = VisualType::CAMERA_FRAMES;

    if (ImGui::Selectable(enumToCharP(VisualType::AXIS_WITH_LINE),
                          type == VisualType::AXIS_WITH_LINE))
      type = VisualType::AXIS_WITH_LINE;

    if (ImGui::Selectable(enumToCharP(VisualType::CAMERA_FRAMES_WITH_LINE),
