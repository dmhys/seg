...
struct primitives {
  /**
   * @brief Returns grid with given Grid / Cell size. Cell starts tiling from
   * (0,0)
   ...
   */
  static std::vector<Eigen::Vector3f> Grid(Eigen::Vector2f max,
                                           Eigen::Vector2f min,
   ...
   * @return vertices of Grid to use with GL_LINE
   */
  static inline std::vector<Eigen::Vector3f> Grid(float grid_size,
                                                  float cell_size) {
    return Grid(Eigen::Vector2f(grid_size, grid_size),
                Eigen::Vector2f(-grid_size, -grid_size),
   ...
   */
  static std::tuple<std::vector<Eigen::Vector3f>, std::vector<Eigen::Vector3f>>
  Axis(float axis_length = 1.0f);
  ...
  static std::vector<Eigen::Vector3f> GaussianRandomVertices(int sample_count,
                                                             float stddev);
   ...
   */
  static std::tuple<std::vector<Eigen::Vector3f>, std::vector<Triangle>>
  Icosahedron(float edge_length = -1.0f);

  /**
   * @brief Generates sphere, using subdivision iteration starting from
   * icosahedron.
   * @param radius - radius of sphere
   * @param detail_level - Iteration number of subdivision. The more, the smooth
   * sphere will be. detail_level 0 means no iteration, resulting icosahedron.
   * @return tuple - <1> std::vector<Eigen::Vector3f> vertices x/y/z
   *         tuple - <2> std::vector<Triangle> vertex index of each faces
   */
  static std::tuple<std::vector<Eigen::Vector3f>, std::vector<Triangle>>
  Icosphere(float radius, int detail_level = 3);
   ...
   */
  static std::vector<Eigen::Vector3f> CameraFrame(float aspect = 1.5,
                                                  float scale = 1.0f);
   ...
   * @return Line Segments. (8 lines)
   */
  static std::vector<Eigen::Vector3f> CameraFrame(float width,
                                                  float height,
                                                  float focal_length,
                                                  float scale = 1.0f);

  /**
   * @brief Returns camera frame Line Segments to render, from camera
   * intrinsics.
   * @return Line Segments. (8 lines)
   */
  static std::vector<Eigen::Vector3f> CameraFrame(float width,
