...
std::vector<Eigen::Vector3f> generate(
    float left, float right, float upper, float lower, float scale) {
...
std::vector<Eigen::Vector3f> primitives::Grid(Eigen::Vector2f max,
                                              Eigen::Vector2f min,
  ...
  if (max.x() < cell_size.x() || max.y() < cell_size.y() ||
      -cell_size.x() < min.x() || -cell_size.y() < min.y())
    throw std::invalid_argument(
  ...
  const Eigen::Vector2f max_xy(max_uv.x() * cell_size.x(),
                               max_uv.y() * cell_size.y());
  const Eigen::Vector2f min_xy(min_uv.x() * cell_size.x(),
                               min_uv.y() * cell_size.y());

  std::vector<Eigen::Vector3f> grid;
  grid.reserve((max_uv.x() - min_uv.x() + 1) * 2 +
               (max_uv.y() - min_uv.y() + 1) * 2);
...
primitives::Axis(float axis_length) {
  std::vector<Eigen::Vector3f> vertices;
...
std::vector<Eigen::Vector3f> primitives::GaussianRandomVertices(
  ...
  for (int i = 0; i < sample_count; i++)
    vertices.push_back(Eigen::Vector3f(distrib(gen) + mean.x(),
                                       distrib(gen) + mean.y(),
                                       distrib(gen) + mean.z()));
...
std::vector<Eigen::Vector3f> primitives::GaussianRandomVertices(
    int sample_count, float stddev) {
  return primitives::GaussianRandomVertices(sample_count,
                                            Eigen::Vector3f::Zero(), stddev);
}

std::tuple<std::vector<Eigen::Vector3f>, std::vector<Triangle>>
primitives::Icosahedron(float edge_length) {
  if (edge_length < 0)
...
std::tuple<std::vector<Eigen::Vector3f>, std::vector<Triangle>>
primitives::Icosphere(float radius, int detail_level) {
  if (radius < 0)
    throw std::invalid_argument(
        "primitives::icosphere - given radius is negative.");

  if (detail_level == 0)
    return primitives::Icosahedron(icosahedron::l * radius);

  if (5 < detail_level) {
    LOG_INFO(
        "primitives::icosphere max detail_level is 5, which generates enough "
        "detail "
        "- 10242 "
  ...
  for (int i = 0; i < detail_level; i++) {
    ...
    for (auto &&triangle : triangles)  // each triangle
    {
      ...
      new_triangles.push_back(Triangle{triangle.vertex_index[0],
                                       midle_vertex_id[0], midle_vertex_id[2]});
      new_triangles.push_back(Triangle{triangle.vertex_index[1],
                                       midle_vertex_id[1], midle_vertex_id[0]});
      new_triangles.push_back(Triangle{triangle.vertex_index[2],
                                       midle_vertex_id[2], midle_vertex_id[1]});
