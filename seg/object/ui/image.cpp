...
void getImageFitPosAndSize(int image_width,
  ...
  {
    const int disp_size_x =
        static_cast<int>(content_area_size.y * aspect_image);
  ...
  {
    const int disp_size_y =
        static_cast<int>(content_area_size.x / aspect_image);
    const int margin_updown =
        static_cast<int>(content_area_size.y) - disp_size_y;
    image_cursor_pos =
...
void Image::setData(const cv::Mat& image) { setData(image.clone()); }
...
void Image::updateTexture() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, image_buffer.cols, image_buffer.rows,
               0, GL_RGB, GL_UNSIGNED_BYTE, image_buffer.ptr());
